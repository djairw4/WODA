...
void loop() {
  ...
  //Get pH
  pH_raw=analogRead(SensorPin);
  //pH=(float)(pH_raw-50)*5.0/662+4.0;
  pH=(float)(temp-412)*2.0/300+7.0;  
  Serial.print("pH: ");  
  ...
  client.publish(mqtt_topic, mqttbuffer);
   ...
   // Publish pH
  sprintf(mqttbuffer, "{ \"idx\" : %d, \"nvalue\" : 0, \"svalue\" : \"%2.1f\" }", idx2, pH);
  Serial.print(mqttbuffer);
  Serial.print("\n");
  client.publish(mqtt_topic, mqttbuffer);
